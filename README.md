...
- `++Nd` - повтор каждые N дней (умный, сохраняет день недели)
- `.+Nd` - повтор через N дней от даты завершения
- `+Nw` - повтор каждые N недель
- `+Nm` - повтор каждые N месяцев
- `+Ny` - повтор каждые N лет
- `+Nwd` - **повтор каждые N рабочих дней** (с учетом праздников и выходных РФ)
